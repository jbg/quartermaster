...
- **Reminders are a durable inbox, not fetch-and-disappear alerts.** `GET /reminders` returns due, unacked reminders even after first presentation. `presented_at` tracks first presentation; `acked_at` is the explicit dismissal state. Don't auto-ack on read/poll.
