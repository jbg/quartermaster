...
- **TEXT-backed enums at the DB boundary, typed enums at the API boundary.** `product.source`, `product.family`, `stock_event.event_type` are `TEXT` columns (for `sqlx::Any` portability and easy migrations). DTOs convert to typed Rust enums (`ProductSource`, `UnitFamily`, `StockEventType`) at the edge so the OpenAPI spec and generated clients get real enums. Keep this split: don't push enum types into the DB layer.
...
- **Auth has two client contracts.** Native/generated clients use opaque bearer access + refresh tokens from the JSON auth responses. The browser web client uses HttpOnly auth cookies plus a readable `qm_csrf` double-submit cookie/header for unsafe requests. Keep authenticated handlers compatible with both paths; don't make cookie-only assumptions in shared API code.
...
- **Dependency policy:** `cargo deny check advisories bans licenses sources` mirrors CI's Rust dependency gate.
...
- **Web dynamic routes are first-class SPA routes.** `/products`, `/products/new`, `/products/{id}`, `/products/{id}/edit`, `/products/{id}/delete`, and `/batches/{id}` are refresh-safe frontend routes. Because adapter-static cannot prerender arbitrary product or batch IDs, `svelte.config.js` ignores unseen routes and the Rust web fallback serves `200.html`; keep future dynamic web routes on that model unless they need server ownership.
