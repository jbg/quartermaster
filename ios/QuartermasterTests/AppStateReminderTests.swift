...
final class AppStateReminderTests: XCTestCase {
  ...
  func testUserInitiatedRefreshClearsPriorErrorAndSurfacesFreshFailure() async {
    let api = FakeAPI(
      reminderResponses: [
        .failure(APIError.server(status: 503, body: apiErrorBody(message: "Fresh failure")))
      ]
    )
    let appState = makeAppState(api: api)
    appState.phase = .authenticated(me())
  ...
  func testAcknowledgeReminderRestoresStateOnFailureAndClearsInFlight() async {
    ...
    let api = FakeAPI(
      ackResponses: [
        .failure(APIError.server(status: 503, body: apiErrorBody(message: "Ack failed")))
      ]
    )
    let appState = makeAppState(api: api)
    appState.phase = .authenticated(me())
  ...
  func testOpenReminderFailureStillKeepsFallbackInventoryTarget() async throws {
    ...
    let api = FakeAPI(
      ...
      openResponses: [
        .failure(APIError.server(status: 502, body: apiErrorBody(message: "Open failed")))
      ]
    )
    let appState = makeAppState(api: api)
    appState.phase = .authenticated(me())
...
private actor FakeAPI: AppStateAPI {
  ...
  func registerDevice(
    deviceID: String, pushToken: String?, pushAuthorization: PushAuthorizationStatus,
    appVersion: String?
  ) async throws {}
