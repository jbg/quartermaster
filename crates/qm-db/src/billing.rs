...
pub async fn create(db: &Database, plan_key: &str) -> Result<BillingAccountRow, sqlx::Error> {
    ...
    let row = create_in_tx(&mut tx, plan_key).await?;
    tx.commit().await?;
    Ok(row)
}
...
pub async fn ensure_for_household(
    ...
    let mut tx = db.pool.begin().await?;
    let row = ensure_for_household_in_tx(&mut tx, household_id, plan_key).await?;
    tx.commit().await?;
    Ok(row)
}
...
pub async fn ensure_for_household_in_tx(
    ...
    if let Some(row) = find_for_household_in_tx(&mut *tx, household_id).await? {
        return Ok(row);
    }
    let account = create_in_tx(&mut *tx, plan_key).await?;
    attach_household_in_tx(&mut *tx, household_id, account.id).await?;
    Ok(account)
}
...
pub async fn attach_household(
    ...
    sqlx::query("UPDATE household SET billing_account_id = ? WHERE id = ?")
